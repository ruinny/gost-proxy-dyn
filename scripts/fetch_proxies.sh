#!/bin/sh
# fetch_proxies.sh �?�?Webshare API 获取代理列表，生�?Gost V3 配置文件
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] 正在�?Webshare API 拉取代理列表..."

# ── 拉取代理列表（分页，最多获�?MAX_PROXIES 个节点）──────────
# 每页最�?100 个，计算需要的页数
PAGE_SIZE=100
...
while [ "$i" -le "$PAGES" ]; do
  ...
  if [ -z "$RESPONSE" ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] �?�?${i} 页请求失败，跳过�?
    break
  fi

  # 提取该页节点并合�?  PAGE_NODES=$(echo "$RESPONSE" | jq -c '.results // []')
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] �?${i} 页获取到 ${COUNT} 个节点，累计 ${TOTAL_FETCHED} 个�?

  # 如果已够数量或者返回节点数小于 PAGE_SIZE（最后一页），停�?  if [ "$TOTAL_FETCHED" -ge "$MAX_PROXIES" ] || [ "$COUNT" -lt "$PAGE_SIZE" ]; then
    break
  fi
  i=$((i + 1))
done
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] �?获取�?${PROXY_COUNT} �?${PROXY_COUNTRY} 代理节点�?
...
# Gost V3 动态代理池配置
# 自动生成�?$(date '+%Y-%m-%d %H:%M:%S')，代理数量：${PROXY_COUNT}
services:
  - name: socks5-ingress
    addr: ":1080"
    handler:
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] �?Gost 配置已写�?${GOST_CONFIG}�?
